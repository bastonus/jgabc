  ...
  <div>\
    ...
    <div class='btn-group'>\
      ...
      function playNextNote (time){
        ...
        if (!noteElem || !noteElem.isConnected) {
          var targetSvg = originalSvg || score.svg;
          if (targetSvg && targetSvg.querySelector) {
            if (note.elementIndex !== undefined) {
              noteElem = targetSvg.querySelector('use[element-index="' + note.elementIndex + '"]');
            }
            if (!noteElem && note.sourceIndex !== undefined) {
              noteElem = targetSvg.querySelector('use[source-index="' + note.sourceIndex + '"]');
            }
          }
        }
