  ...
  var tones = {
    ...
    playFrequency: function (freq, options) {
      ...
      var stopAndDisconnect = function () {
        try {
          osc.stop(0);
        } catch (e) { /* Ignore if already stopped or never started */ }
        try {
    ...
    play: function (freqOrNote, octave, transpose) {
      if (typeof freqOrNote === "number") {
        ...
        var index = octave * 12 + this.octaveMap[freqOrNote.toLowerCase()];
        var freq = this.map[index];
        if (typeof freq === 'number' && isFinite(freq)) {
          this.playFrequency(freq);
        } else {
          console.warn('tones.play: Invalid frequency for note', freqOrNote, 'in octave', octave);
        }
