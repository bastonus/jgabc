/**
 * =========================================================================
 * Oremus - gregorian_search_engine.js
 * Moteur universel et ultra-rapide de recherche liturgique & grégorienne
...
(function(root) {
    ...
    var LITURGICAL_SYNONYMS = {
        ...
        'saint jean': ['joannis', 'joannes', 'in moyen de l\'ecclesiae', 'in medio ecclesiae'],
    ...
    var GregorianSearchEngine = {
        ...
        buildShowcaseList: function() {
            ...
            for (var k = 0; k < this.chantsList.length; k++) {
                pushItem(this.chantsList[k]);
            }

            this.defaultShowcaseList = showcase;
        },

        buildIndex: function(data) {
            if (!Array.isArray(data)) return;
            this.chantsList = data;
            this.chantsMap = {};
            this.tokenInvertedIndex = new Map();
            this.stemInvertedIndex = new Map();
            ...
            for (var i = 0; i < this.chantsList.length; i++) {
                ...
                item.normFullTextLa = normalizeLatin(item.fullTextLa || item.incipit || '');
                ...
                for (var j = 0; j < words.length; j++) {
                    ...
                    if (!seenTokens.has(w)) {
                        seenTokens.add(w);
                        var arr = this.tokenInvertedIndex.get(w);
                        if (!arr) {
                            arr = [];
                            this.tokenInvertedIndex.set(w, arr);
                        }
                        arr.push(i);
                    }
                    ...
                    if (st.length >= 3 && !seenStems.has(st)) {
                        seenStems.add(st);
                        var sarr = this.stemInvertedIndex.get(st);
                        if (!sarr) {
                            sarr = [];
                            this.stemInvertedIndex.set(st, sarr);
                        }
                        sarr.push(i);
                    }
                }
            ...
            this.buildShowcaseList();
            this.isInitialized = true;
        },

        executeSearch: function(query, filters, limit, offset) {
            ...
            if (!query && !filterPart && !filterMode) {
                var sourceList = this.defaultShowcaseList.length > 0 ? this.defaultShowcaseList : this.chantsList;
                ...
                return {
                    ...
                    totalCount: this.chantsList.length,
            ...
            var scores = new Float32Array(this.chantsList.length);
            var matchedSet = new Uint8Array(this.chantsList.length);

            // 0. Recherche biblique directe
            if (bibleRef) {
                for (var b = 0; b < this.chantsList.length; b++) {
                    var bItem = this.chantsList[b];
            ...
            if (synonyms.length > 0) {
                for (var s = 0; s < synonyms.length; s++) {
                    ...
                    for (var st = 0; st < synTokens.length; st++) {
                        ...
                        var list = this.tokenInvertedIndex.get(stok);
                        if (list) {
                            for (var l = 0; l < list.length; l++) {
                                var idx = list[l];
                                var item = this.chantsList[idx];
            ...
            for (var t = 0; t < qTokens.length; t++) {
                var tok = qTokens[t];
                var stem = qStems[t];
                var tokList = this.tokenInvertedIndex.get(tok);

                if (tokList) {
                    for (var k = 0; k < tokList.length; k++) {
                        var tIdx = tokList[k];
                        var tItem = this.chantsList[tIdx];
                ...
                if (stem.length >= 3) {
                    var stemList = this.stemInvertedIndex.get(stem);
                    if (stemList) {
            ...
            var candidates = [];
            for (var c = 0; c < this.chantsList.length; c++) {
                if (!matchedSet[c] && normQ) continue;

                var chant = this.chantsList[c];
        ...
        getById: function(id) {
            return this.chantsMap[id] || null;
        }
    };

    root.GregorianSearchEngine = GregorianSearchEngine;
