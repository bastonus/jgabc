...
def update_youtube_recording(chant_id: str, new_yt_id: str, title: str = "", channel: str = "", duration: str = "", flag_reason: str = None):
    ...
    # Mettre à jour lab_data.js si la pièce y figure
    if os.path.exists(LAB_DATA_JS):
        try:
            with open(LAB_DATA_JS, "r", encoding="utf-8") as f:
                raw = f.read()
            ...
            print(f"[OK] lab_data.js synchronisé avec nouveau YouTube pour chant {chant_id}")
        except Exception as e:
            print(f"[WARN] Impossible de mettre à jour lab_data.js : {e}")
...
def save_note_timestamps(chant_id: str, timestamps_list: list):
    ...
    if os.path.exists(LAB_DATA_JS):
        try:
            ...
            with open(LAB_DATA_JS, "w", encoding="utf-8") as f:
                f.write("const PIECES = " + json.dumps(pieces, ensure_ascii=False, indent=2) + ";\n")
            print(f"[OK] lab_data.js synchronisé pour chant {chant_id}")
        except Exception as e:
            print(f"[WARN] Impossible de mettre à jour lab_data.js : {e}")
