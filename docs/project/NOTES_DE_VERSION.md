...
### 📑 Sommaire Dynamique de la Messe & ScrollSpy Intelligent

* **Navigation Structurée pour la Messe (`#doMassTocPanel`) :**
  * **Affichage Permanent sur Ordinateur & Tablettes (>900px) :** Intégration d'un volet latéral transparent et épuré (`do-mass-toc-panel`), sans masquer le contenu liturgique.
  * **Pastille Flottante Mobile (`#doMassTocPill`) :** Sur écran mobile (≤900px), une pastille ergonomique avec effet de flou glassmorphism au-dessus du lecteur audio permet d'ouvrir le volet en tiroir modal avec voile d'arrière-plan.
  * **ScrollSpy en Temps Réel :** Suivi dynamique de la section de la messe en cours de lecture, mise en surbrillance automatique et défilement autonome (`auto-scroll`) de la liste du sommaire pour garder la partie active visible.
  * **Saut Instantané & Pré-rendu :** Clic sur une partie (Introït, Kyrie, Gloria, Collecte, Épître, Graduel, Évangile, Credo, Offertoire, Préface, Canon, Communion...) avec défilement fluide et pré-rendu instantané des partitions GABC pour éviter tout décalage visuel.
  * **Commutateurs Rapides :** Boutons d'accès direct intégrés au pied du sommaire pour basculer instantanément l'affichage du **Chant Grégorien** et de **l'Ordinaire Complet** sans repasser par les menus généraux.

---

### 🎥 Enrichissement Majeur du Répertoire Grégorien (14 000+ Enregistrements YouTube)

* **Catalogue Étendu & Fiabilisé (`js/gregorian_youtube_links.js`) :**
  * Intégration massive de correspondances directes avec les enregistrements d'autorité du projet *Graduale Project* (Marek Klein) et des répertoires grégoriens traditionnels.
  * Association stricte par incipit et identifiant GregoBase pour un accès direct depuis le tiroir des sources du lecteur audio.

---

### 🖐️ Fluidité Gestuelle & Améliorations de l'Expérience Utilisateur (UX)

* **Physique Améliorée du Tiroir des Pièces de Messe (`#massPartPickerDrawer`) :**
  * Fermeture naturelle par inertie de glissement (`velocity > 0.22`) ou glissement vers le bas supérieur à 15% de la hauteur du tiroir.
  * Élimination des soubresauts d'animation grâce à des transitions CSS cubiques optimisées (`cubic-bezier(0.2, 0.9, 0.3, 1)`).
  * Retours haptiques calibrés lors des transitions d'état.

---

### 📁 Architecture & Nettoyage Complet du Dépôt

* **Arborescence Réorganisée & Préservation Totale :**
  * Plus de 80 fichiers isolés ou temporaires (retours utilisateurs, anciens scripts, fichiers logs, sources d'extraction) ont été organisés dans des dossiers thématiques clairs (`feedback/`, `releases/`, `logs/`, `legacy_tools/`, `crampon/source/`, `psalms/reference/`, etc.).
  * Aucun fichier supprimé, traçabilité Git intégrale (`git mv`).
  * Module externe `gabc.pack` re-compilé et enrichi comprenant 25 290 partitions GABC.

---

### 🛡️ Versionnage & Maintenance

* Synchronisation de `CURRENT_APP_VERSION` sur `'beta-0.0.58'`, `versionCode 58` / `versionName "beta-0.0.58"` (`android/app/build.gradle`), `version.json` (`tagName v0.0.58`, `releaseDate 2026-09-15T12:45:00Z`), `package.json` (`0.0.58`) et synchronisation du dossier Android `www/`.
* Incrément du cache Service Worker `oremus-pwa-v1.3.26 → v1.3.27` (`sw.js` et `www/sw.js`).
* Mise à jour du flux distant et local de notifications (`notifications.json`).
